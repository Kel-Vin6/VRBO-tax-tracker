...
public enum DateMath {
    ...
    public nonisolated static func startOfDay(_ date: Date) -> Date {
        calendar.startOfDay(for: date)
    }

    /// Nights between two dates, which is what a lodging platform bills.
    /// A same-day check-in/check-out counts as zero nights.
    public nonisolated static func nights(from checkIn: Date, to checkOut: Date) -> Int {
    ...
    public nonisolated static func inclusiveDays(from start: Date, to end: Date) -> Int {
        let a = startOfDay(start)
        let b = startOfDay(end)
        return max(0, (calendar.dateComponents([.day], from: a, to: b).day ?? 0) + 1)
    }

    public nonisolated static func year(of date: Date) -> Int {
        calendar.component(.year, from: date)
    }

    public nonisolated static func month(of date: Date) -> Int {
        calendar.component(.month, from: date)
    }

    public nonisolated static func startOfYear(_ year: Int) -> Date {
    ...
    public nonisolated static func endOfYearExclusive(_ year: Int) -> Date {
        calendar.date(from: DateComponents(year: year + 1, month: 1, day: 1)) ?? Date()
    }

    public nonisolated static func yearInterval(_ year: Int) -> DateInterval {
        DateInterval(start: startOfYear(year), end: endOfYearExclusive(year))
    }

    public nonisolated static func contains(_ date: Date, inYear year: Int) -> Bool {
    ...
    public nonisolated static func nights(from checkIn: Date, to checkOut: Date, inYear year: Int) -> Int {
    ...
    public nonisolated static func days(from start: Date, to end: Date, inYear year: Int) -> Int {
    ...
    public nonisolated static func daysInYear(_ year: Int) -> Int {
        let start = startOfYear(year)
        let end = endOfYearExclusive(year)
        return calendar.dateComponents([.day], from: start, to: end).day ?? 365
    }

    public nonisolated static func adding(days: Int, to date: Date) -> Date {
        calendar.date(byAdding: .day, value: days, to: date) ?? date
    }

    public nonisolated static func adding(months: Int, to date: Date) -> Date {
        calendar.date(byAdding: .month, value: months, to: date) ?? date
    }

    /// Two ranges overlap if they share at least one calendar day.
    public nonisolated static func rangesOverlap(_ aStart: Date, _ aEnd: Date, _ bStart: Date, _ bEnd: Date) -> Bool {
        startOfDay(aStart) <= startOfDay(bEnd) && startOfDay(bStart) <= startOfDay(aEnd)
    }

    public nonisolated static var currentYear: Int { year(of: Date()) }

    /// Tax years the app offers for selection: a decade back through next year.
    public nonisolated static func selectableYears(around year: Int = currentYear) -> [Int] {
        Array((year - 9)...(year + 1)).reversed()
    }
}

public extension Date {
    nonisolated var taxYear: Int { DateMath.year(of: self) }
    nonisolated var startOfDay: Date { DateMath.startOfDay(self) }
}
